...
### 21/09/2026 (cont. 6) — Verificação para produção em VPS (clone limpo do GitHub, NODE_ENV=production)

Testado num clone limpo do repositório: `npm ci --omit=dev` e `pip install -r requirements.txt` funcionam; o servidor sobe em produção, cria `storage/` sozinho e monta as tabelas; login com a senha do `.env` OK (200) e as quatro senhas antigas retornam 401; `/api/settings/prompts` carrega o `oraculo-v2`; chat do Criador (`/api/criador/stream`, 5 lâminas, Bella Essencial) entrega 5 lâminas com layouts idênticos ao plano, `DIREÇÃO_JSON` válido (21/21 chaves), CTA e caption, sem scorecard (US$ 0,10); geração real de 2 artes em qualidade baixa terminou em ~20 s e gravou em `storage/carousels/`. **Corrigido no caminho:** (1) senhas de super admin escritas no código (`fonteoculta2024`, `FonteOculta@2025`, `admin@exemplo.com.br`/`senha_ficticia_123`) removidas de `routes/auth.js` e `state.js`; `db.js` não cria mais o admin de desenvolvimento em produção; (2) caminhos fixos `/app/backend/storage/...` trocados por relativos ao código; (3) o modo Oráculo único perdia o plano de layouts e o `DIREÇÃO_JSON` que a rota do chat injetava no prompt `criador` (o gerador de imagem lê `visual_plan`): agora vão como "CONTRATO TÉCNICO DAS LÂMINAS" na direção visual. **NÃO testado:** Postgres real (o teste caiu no PGlite), `python3` do venv via pm2 no Linux, nginx/pm2, RabbitMQ, MinIO/B2. Ver `DEPLOY_VPS.md`. Pendências conhecidas: defaults fracos `oraculo_pass_123` (RabbitMQ) e `oraculo_secret_123` (MinIO) só valem se esses serviços existirem sem variável definida.

### 21/09/2026 (cont. 5) — Repositório no GitHub (`jords024/oraculo-bella`, PRIVADO)

Até aqui a pasta do projeto não era um repositório git e nenhum repo do GitHub tinha o sistema da Bella (`Oraculo`, `nano-banana-mcp` e `oraculo-haucacau` são HauCacau/Fonte Oculta e **públicos**). Foi criado `jords024/oraculo-bella` (privado), branch `main`, 677 arquivos. O `.git` fica **fora do OneDrive**, em `C:/Users/julia/git-repos/oraculo-bella.git` (a pasta do projeto tem só um arquivo `.git` apontando para lá). Push: `git -c credential.helper='!gh auth git-credential' push` (sem alterar a config global). `.gitignore` na raiz é uma lista branca: só `backend/` e `frontend/`; fora ficam `.env`, `.mcp.json` (tem chave do Gemini), `storage/` (banco e imagens), `dashboard/data/*` (exceto `branding.json`), `*.docx` (contratos), `node_modules`, logs e `backend/generate-slides.mjs`. `frontend/dist` é versionado porque o backend serve `../frontend/dist`. **ALERTA DE SEGURANÇA:** `generate-slides.mjs` tem uma chave de API do Google escrita no código e essa MESMA chave está nos 3 repositórios públicos antigos (`Oraculo`, `nano-banana-mcp`, `oraculo-haucacau`); precisa ser revogada e trocada no Google Cloud. **Para subir em produção:** as variáveis do `.env` (OPENAI_API_KEY, DASHBOARD_*, JWT_SECRET etc.) são configuradas no Render/Railway, não vão no repo; `ORACULO_MODO_UNICO` é ligado por padrão; conferir em Settings de produção se não há prompts salvos no banco (`agent_prompts`), pois eles sobrescrevem os `.md` e o Oráculo novo não valeria.
